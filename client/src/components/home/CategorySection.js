...
const CategorySection = (props) => {
    ...
    return items.length !=0 && (
        <Grid item xs={12} md={12} sm={12} my={2}>
            ...
            <Grid container>
                {items.slice(currentPage * RECORDS_PER_PAGE, (currentPage * RECORDS_PER_PAGE) + RECORDS_PER_PAGE).map((menuItem) => (
                    <Grid
                        item
                        xs={12}
                        sm={4}
            ...
            <Grid container>
                <Grid item xs={12} md={12} sm={12} textAlign="center">
                    {[...Array(totalPages).keys()].map((page) => (
                        <Button key={page} variant={page === currentPage ? 'contained' : 'outlined'} sx={{mx: 0.5}} onClick={() => handlePageChange(page)}>{page + 1}</Button>
                    ))}
                </Grid>
            </Grid>
        </Grid>
